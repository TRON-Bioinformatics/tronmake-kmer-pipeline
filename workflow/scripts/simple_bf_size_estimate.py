...
def main():
    ...
    if distinctKmers == None:
        usage("you have to tell me how many distinct kmers will be in the bloom filter")
    if fpRate == None:
        usage(
            "you have to give me the desired false positive rate for the bloom filter"
        )
    ...
    if numHashFuncs == 1:
        ...
        numBits = -(numHashFuncs * distinctKmers) / log(
            1 - (fpRate) ** (1 / float(numHashFuncs))
        )
...
def int_with_unit(s):
    ...
    try:
        ...
        return int(ceil(float(s) * multiplier))
