...
rule collect_graphs:
    input:
        cdbg=expand("index/bcalm2/graphs/{sample}.unitigs.fa.gz", sample=samples.bin_id),
    output:
...
"""
    ...
    run:
        with (
            open(input.sample_sheet, "r") as file_handle,
            open(output.fof, "w") as write_handle,
            open(output.index_mapping, "w") as mapping_handle,
        ):
            ...
            for line in file_handle:
                ...
                minimiser_id = os.path.basename(fastq.split(" ")[0]).rstrip(
                    ".minimiser"
                )
